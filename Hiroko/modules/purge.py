...
async def purge(hiroko: Hiroko, msg: Message):
    ...
    if msg.reply_to_message:
        ...
        try:
            for plist in m_list:
                await hiroko.delete_messages(chat_id=msg.chat.id, message_ids=plist, revoke=True)
...
async def spurge(hiroko: Hiroko, msg: Message):
...
async def del_msg(hiroko: Hiroko, msg: Message):
    if msg.chat.type != ChatType.SUPERGROUP:
        await msg.reply_text(text="**ɪ ᴄᴀɴ'ᴛ ᴘᴜʀɢᴇ ᴍᴇssᴀɢᴇs ɪɴ ᴀ ʙᴀsɪᴄ ɢʀᴏᴜᴘ ᴍᴀᴋᴇ sᴜᴘᴇʀ ɢʀᴏᴜᴘ.**")
        return        
    if msg.reply_to_message:
