...
def select_character(_, message):
    ...
    if message.chat.type == enums.ChatType.PRIVATE:
        if user_id in user_profiles:
            ...
            # Insert the initial user profile data into MongoDB
            collection.insert_one(user_profiles[user_id])

        message.reply_photo(
            photo="https://telegra.ph/file/061d8efe5247272458cb0.jpg",
            caption="Welcome to the Jujutsu Kaisen fighting game! Choose your character:",
        ...
        message.reply_text(
            f"ʜᴇʏ {message.from_user.mention} ᴘᴍ ᴍᴇ ɪғ ʏᴏᴜ ɪɴᴛᴇʀᴇsᴛᴇᴅ.",
            reply_markup=btn
        )
...
def handle_character_selection(_, query):
    ...
    if user_id not in user_profiles:
        return  # Do nothing if the user profile doesn't exist
    ...
    if character_id in characters:
        ...
        # Update the user profile data in MongoDB
        collection.update_one({"user_id": user_id}, {"$set": {"character": character_id}})
        # No need to insert the entire user profile data again
